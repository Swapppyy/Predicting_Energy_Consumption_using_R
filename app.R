...
library(shiny)
library(tidyverse)
transformed_data <- read_csv("C:\Users\DELL\Desktop\SY Material\IST 689 - ADS\Final Project\combined_data.csv")
...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(
            ...
            br(),
            # 1 Hour
            sliderInput("Hour", "Select an hour:", min = 1, max = 24, value = 12),
...
server <- function(input, output) {
    ...
    output$histogram <- renderPlot({
        ggplot(selected_data(), aes(x = hour, y = get(input$column), fill = as.factor(hour))) + geom_bar(stat = "identity", position = "dodge", color = "grey", alpha = 0.7) + labs(title = paste("Histogram of", input$column, "with respect to hours"), x = "Hour", y = input$column) + theme_minimal()
    })
